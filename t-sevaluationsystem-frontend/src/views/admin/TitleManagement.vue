    ...
    <div class="search-area">
      <div class="search-row">
        <div class="search-item">
          <label>职称名称</label>
          <input v-model="searchForm.titleName" placeholder="请输入职称名称" @keyup.enter="handleSearch" />
        </div>
      ...
      <div class="modal-content" @click.stop>
        ...
        <div class="modal-body">
          <div class="form-row">
            <div class="form-col">
              <div class="form-item">
                <label>职称名称 <span class="required">*</span></label>
                <input v-model="form.titleName" placeholder="请输入职称名称" />
              </div>
            </div>
          </div>
...
import { addTitleApi, listTitleApi, updateTitleApi, deleteTitleApi } from '@/api/title.js'
...
const fetchList = async () => {
  try {
    const res = await listTitleApi(searchForm.value, pageNum.value, pageSize.value)
    if (res.code === 1) {
...
const handleDeleteConfirm = async () => {
  try {
    const res = await deleteTitleApi(deleteId.value)
    if (res.code === 1) {
...
const handleSubmit = async () => {
  ...
  try {
    const res = isEdit.value
      ? await updateTitleApi(form.value)
      : await addTitleApi(form.value)
    if (res.code === 1) {
